...
const buildNodeToJson = (graph: Graph, node: Node): ELJson | undefined => {
  ...
  // for 返回
  const forGroup: { [key: string]: ELJson[] } = {};
  const forJson: ELCondition = {
    conditionType: ConditionType.for,
    executeType: ExecuteType.CONDITION,
    executableGroup: forGroup,
  };
  ...
  // for port 端点
  const forKeyGroupList = [] as ELJson[];
  forGroup.FOR_KEY = forKeyGroupList;
  const data = node.getData() as NodeData;
  const forKeyJson: ELNode = {
    id: data.liteInfo.id,
    type: NodeShapes.FOR,
  ...
  forKeyGroupList.push(forKeyJson);
  ...
  if (doNodes) {
    ...
    if (doKey) {
      forGroup.DO_KEY = [unsealThenWrapper(doKey)];
    }
  }
  ...
  if (breakNodes) {
    ...
    forGroup.BREAK_KEY = breakKeyGroupList;
  }
  ...
  return nextWrapper(graph, forJson, nextNodes);
};
